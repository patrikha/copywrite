...
fn get_type_settings(language: &Option<String>) -> HashMap<String, Language> {
    let type_settings: HashMap<String, Language> = {
        let mut t = HashMap::new();
        t.insert("c".to_string(), create_c_style_language(osvec!["c", "cc", "h"]));
        t.insert("cpp".to_string(), create_c_style_language(osvec!["cpp", "hpp", "cxx", "hxx", "ixx"]));
        ...
        t.insert("php".to_string(), create_c_style_language(osvec!["php", "phtml", "php3", "php4", "php5", "php7", "phps", "php-s", "pht", "phar"]));
        t.insert("typescript".to_string(), Language {
        ...
        t.insert("python".to_string(), Language {
            extensions: osvec!["py"],
            keep_first: Some(Regex::new(r"^#!|^# +pylint|^# +-\*-|^# +coding|^# +encoding|^# +type|^# +flake8").unwrap()),
        ...
        t.insert("proto".to_string(), Language {
        ...
        t.insert("html".to_string(), Language {
        ...
        t.insert("css".to_string(), Language {
        ...
        t.insert("script".to_string(), Language {
    ...
    if let Some(lang) = language {
        ...
        if filtered_type_settings.is_empty() {
            log::error!("Language {} is not supported.", lang);
            exit(10);
        }
...
lazy_static! {
    static ref YEARS_PATTERN: Regex = RegexBuilder::new(r"(Copyright\s*(?:\(\s*[Cc©]\s*\)\s*))?([0-9][0-9][0-9][0-9](?:-[0-9][0-9]?[0-9]?[0-9]?)?)").case_insensitive(true).build().unwrap();
...
fn read_content(path: &Path) -> io::Result<Content> {
    ...
    if bom == Bom::Utf16Be || bom == Bom::Utf16Le || bom == Bom::Utf32Be || bom == Bom::Utf32Le {
        return Err(io::Error::new(io::ErrorKind::Unsupported, "copywrite don't support UTF16/32 encoded files."));
    }
...
fn find_license(path: &Path, lang_type: &str, settings: &Language) -> Option<License> {
    ...
    for line in content.lines.iter() {
        if (i == 0 || i == skip) && settings.keep_first.is_some() && settings.keep_first.as_ref().unwrap().is_match(line) {
            skip = i + 1;
        } else if EMPTY_PATTERN.is_match(line) {
            // pass
        } else if settings.block_comment_start_pattern.is_some() && settings.block_comment_start_pattern.as_ref().unwrap().is_match(line) {
            head_start = Some(i);
            is_block_header = true;
            break;
        } else if settings.line_comment_start_pattern.is_some() && settings.line_comment_start_pattern.as_ref().unwrap().is_match(line) {
            head_start = Some(i);
            break;
        } else if settings.block_comment_start_pattern.is_none() && settings.line_comment_start_pattern.is_some() && settings.line_comment_start_pattern.as_ref().unwrap().is_match(line) {
            head_start = Some(i);
            break;
        } else {
    ...
    log::debug!("Found preliminary start at {}, i={}, lines={}", head_start.unwrap_or_default(), i, lines_count);
    ...
    if is_block_header {
        ...
        for j in i..lines_count {
            ...
            if LICENSE_PATTERN.is_match(&content.lines[j]) {
                have_license = true;
            } else if settings.block_comment_end_pattern.as_ref().unwrap().is_match(&content.lines[j]) {
                return Some(License {
                    language_type: lang_type.to_string(),
                    content,
                    skip,
        ...
        log::debug!("ELSE1");
        for j in i..lines_count {
            if settings.line_comment_start_pattern.as_ref().unwrap().is_match(&content.lines[j]) && LICENSE_PATTERN.is_match(&content.lines[j]) {
                have_license = true;
            } else if !settings.line_comment_start_pattern.as_ref().unwrap().is_match(&content.lines[j]) {
                log::debug!("ELSE2");
                return Some(License {
                    language_type: lang_type.to_string(),
                    content,
...
fn copywrite_file(path: &Path, lang_type: &str, settings: &Language, template: &[String]) {
    let license: License;
    match find_license(path, lang_type, settings) {
        Some(l) => license = l,
        None => return,
    };
    log::debug!("Info for the file: head_start={:?}, head_end={:?}, have_license={}, skip={}, len={}, years_line={:?}",
                license.head_start, license.head_end, license.have_license, license.skip, license.content.lines.len(), license.years_line);
...
fn is_excluded(entry: &DirEntry, exclude_dirs: &[PathBuf]) -> bool {
    let path = PathBuf::from(entry.path());
    if exclude_dirs.contains(&path) {
        log::info!("Directory {:?} is excluded, skipping.", path);
        return true;
    }
    false
}

pub fn copywrite_path(path: &Path, template: &[String], language: &Option<String>, exclude_dirs: &[PathBuf], exclude_files: &[PathBuf], index_files: &Option<HashSet<OsString>>) {
    log::debug!("Checking path {:?}", path);
    let type_settings = get_type_settings(language);

    for entry in WalkDir::new(&path).follow_links(true).into_iter().filter_entry(|e| !is_excluded(e, exclude_dirs)) {
        let file_path: PathBuf;
        match entry {
            Ok(e) => {
                file_path = e.path().to_path_buf();
            },
            Err(_) => continue,
        }
        ...
        if exclude_files.contains(&file_path) {
            log::info!("File {:?} is excluded, skipping.", file_path);
            continue;
        }
        if let Ok(metadata) = file_path.metadata() {
            log::debug!("checking filesize");
            if metadata.len() == 0 {
        ...
        if let Some(index) = index_files {
            ...
            if cfg!(windows) {
                os_path = OsString::from(str::replace(&file_path.to_string_lossy(), "\\", "/"));
            } else {
                os_path = OsString::from(&file_path);
            }
            if !index.contains(&os_path) {
                log::debug!("{:?} not in git index, skipping.", file_path);
                continue;
            }
        }
